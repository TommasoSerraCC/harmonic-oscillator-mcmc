...
class BlockingSubtab(tk.Frame):
    ...
    def _plot_simple(self, p, kmax_s):
        ...
        for i, (name, lab) in enumerate(zip(obs_names, labels)):
            ax = self.plot.axes[i]
            ax.plot(log2k[sel], bd[name][sel], '-o', ms=4, markerfacecolor='none', markeredgewidth=0.6)
    ...
    def _plot_jack(self, p, kmax_s):
        ...
        for ic, (cn, lab) in enumerate(zip(corr_names, labels)):
            ...
            for nv in bd['n_check']:
                ax.plot(log2k[sel], bd[cn][nv][sel], '-o', ms=4, markerfacecolor='none', markeredgewidth=0.6, label=f'n={nv}')
            ax.set_xlabel(r'$\log_2(k)$')
            ax.set_ylabel(lab)
            ax.legend(fontsize=8)
            apply_grid(ax)
...
class AutocorrSubtab(tk.Frame):
    ...
    def _plot(self):
        ...
        lag = tau_data['lag']
        acf = tau_data['acf']
        ax.plot(lag[1:], acf[1:], 'o', color='black', ms=4, markerfacecolor='none', markeredgewidth=0.6,
                label=r'dati $C_{y^2}$')
    ...
    def _draw_fit_full(self):
        ...
        self.plot.set_layout(1, 1)
        ax = self.plot.axes[0]
        ax.plot(d['lag'][1:], d['acf'][1:], 'o', color='black', ms=4, markerfacecolor='none', markeredgewidth=0.6, label='dati')
    ...
    def _draw_fit_with_residuals(self):
        ...
        ax_main, ax_res = self.plot.set_layout_with_residuals(height_ratios=(3, 1))
        lf = d['lf']
        ax_main.plot(lf, d['af'], 'o', color='black', ms=4, markerfacecolor='none', markeredgewidth=0.6, label='dati')
