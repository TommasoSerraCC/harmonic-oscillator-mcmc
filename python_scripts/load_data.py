...
parser.add_argument('--nt', type=int, default=50, 
                    choices=[4, 12, 24, 30, 36, 42, 50, 75, 100, 150, 200])
args = parser.parse_args()

nt = args.nt
ncorr = nt // 2
ncols_expected = 5 + 4 * ncorr
nsteps = 1000000  # 10^6 steps for final data

data = np.loadtxt(f'data/raw_data_nt{nt}.dat')

# Verifica formato dati
assert data.shape[0] == nsteps, f"Righe attese: {nsteps}, trovate: {data.shape[0]}"
...
print(f"nt={nt}, ncorr={ncorr}, nsteps={nsteps}, shape={data.shape}")
