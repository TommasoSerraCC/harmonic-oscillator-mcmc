...
\begin{equation}
    ...
    \label{eq:overrelax}
\end{equation}
dove $\mu_i$ è la media condizionata definita in precedenza (equazione~\ref{eq:hb_params}).
...
Riportiamo in figura~\ref{fig:combinata} l'andamento di $\langle y \rangle$ e $\langle y^2 \rangle$
...
Possiamo effettuare un fit costante 'naive' sui dati del plateau (figura~\ref{fig:energy_gaps}), per estrarre i valori dei gap
...
Di seguito i risultati ottenuti (tabella~\ref{tab:gap_energetici}):
...
Sono stati utilizzati $3.5 \times 10^4$ campioni di configurazione per ciascun istogramma
scartando i primi $5 \times 10^3$ campioni come fase di termalizzazione. L'unico caso che
...
possiamo ragionevolmente concludere che la catena di Markov implementata campioni correttamente
...
i valori delle osservabili al limite del continuo.
